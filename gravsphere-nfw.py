...
def lnprior_set_single(theta,n_betpars,bet0min,bet0max,\
    ...
    minarr[n_betpars+nu_components*2+2] = logMcenlow
    maxarr[n_betpars+nu_components*2+2] = logMcenhigh
    minarr[n_betpars+nu_components*2+3] = acenlow
    maxarr[n_betpars+nu_components*2+3] = acenhigh
    minarr[n_betpars+nu_components*2+4] = Arotlow
    maxarr[n_betpars+nu_components*2+4] = Arothigh
    minarr[n_betpars+nu_components*2+5] = drangelow
    maxarr[n_betpars+nu_components*2+5] = drangehigh
...
#MUSE
from gravsphere_initialise_AntliaB_test import *
...
M = lambda r, Mpars: \
    mass_nfw(r,Mpars[0],Mpars[1])
rho = lambda r, Mpars: \
    rho_nfw(r,Mpars[0],Mpars[1])
dlnrhodlnr = lambda r, Mpars: \
    nfw_dlnrhodlnr(r,Mpars[0],Mpars[1])
#Central dark mass:
Mcentral = lambda r, Mpars: \
           plummass(r,Mpars[2:4])
n_mpars = 4
...
if (codemode == 'run'):
    ...
    f = open(outdir+'Boutput_chain_nfw.txt','w')
...
#Plotting code:
    ...
    #Read in the data:
    data_in = \
        np.genfromtxt(outdir+'output_surfden_nfw.txt',dtype='f8')
    rbin_phot = data_in[:,0]
    surfden = data_in[:,1]
    surfdenerr = data_in[:,2]
    data_in = \
        np.genfromtxt(outdir+'output_sigp_nfw.txt',dtype='f8')
    rbin_kin = data_in[:,0]
    sigpmean = data_in[:,1]
    sigperr = data_in[:,2]
    ...
    if (propermotion == 'yes'):
        data_in = \
            np.genfromtxt(outdir+'output_prop_nfw.txt',dtype='f8')
        rbin_kinp = data_in[:,0]
        sigpmr = data_in[:,1]
        sigpmrerr = data_in[:,2]
        sigpmt = data_in[:,3]
    ...
    data_in = \
        np.genfromtxt(outdir+'Boutput_chain_nfw.txt',dtype='f8')
    chisq = data_in[:,0]
    ...
    for i in range(nsamples):
        ...
        McenMstore[i] = Mparsu[2]
        Mcenastore[i] = Mparsu[3]
        if (calc_Jfac == 'yes'):
    ...
    plt.savefig(outdir+'output_Sigstar_nfw.pdf',bbox_inches='tight')
    ...
    plt.savefig(outdir+'output_sigLOS_nfw.pdf',bbox_inches='tight')
    ...
    if (propermotion == 'yes'):
        ...
        plt.savefig(outdir+'output_sigpmr_nfw.pdf',bbox_inches='tight')
        ...
        plt.savefig(outdir+'output_sigpmt_nfw.pdf',bbox_inches='tight')
    ...
    plt.savefig(outdir+'output_beta_nfw.pdf',bbox_inches='tight')

    #Write the above data to files for comparitive plotting later:
    f = open(outdir+'output_bet_nfw.txt','w')
    ...
    plt.savefig(outdir+'output_betastar_nfw.pdf',bbox_inches='tight')

    #Write the above data to files for comparitive plotting later:
    f = open(outdir+'output_betstar_nfw.txt','w')
    for i in range(len(rbin)):
        f.write('%f %f %f %f %f %f %f %f\n' % \
    ...
    plt.savefig(outdir+'output_Mass_nfw.pdf',bbox_inches='tight')

    #Write the above data to files for comparitive plotting later:
    f = open(outdir+'output_M_nfw.txt','w')
    for i in range(len(rbin)):
        f.write('%f %f %f %f %f %f %f %f\n' % \
    ...
    f = open(outdir+'output_MdynMstar_nfw.txt','w')
    ...
    #And nu_mass_r:
    f = open(outdir+'output_nu_mass_r_nfw.txt','w')
    for i in range(len(Mstar_rad)):
        f.write('%f %f %f %f %f %f %f %f\n' % \
    ...
    #And Mstar:
    f = open(outdir+'output_Mass_Mstar_nfw.txt','w')
    for i in range(len(Mstar_rad)):
        f.write('%f %f %f %f %f %f %f %f\n' % \
    ...
    f = open(outdir+'output_Mass_Mcen_nfw.txt','w')
    for i in range(len(rbin)):
        f.write('%f %f %f %f %f %f %f %f\n' % \
    ...
    plt.savefig(outdir+'output_rho_nfw.pdf',bbox_inches='tight')

    #Write the above data to files for comparitive plotting later:
    f = open(outdir+'output_rho_nfw.txt','w')
    for i in range(len(rbin)):
        f.write('%f %f %f %f %f %f %f %f\n' % \
    ...
    #And the NFW parameters:
    f = open(outdir+'output_M200c200_chain_nfw.txt','w')
    for i in range(len(M200store)):
        f.write('%f %f %f\n' % \
                (M200store[i],cstore[i],vmaxstore[i]))
    f.close()
    ...
    plt.savefig(outdir+'output_dlnrhodlnr_nfw.pdf',bbox_inches='tight')

    #Write the above data to files for comparitive plotting later:
    f = open(outdir+'output_dlnrhodlnr_nfw.txt','w')
    for i in range(len(rbin)):
        f.write('%f %f %f %f %f %f %f %f\n' % \
    ...
    plt.savefig(outdir+'output_vphirot_nfw.pdf',bbox_inches='tight')
    
    #######################################################
    #Additional write output
    #And write the D+J-factor data:
    if (calc_Jfac == 'yes'):
        f = open(outdir+'output_Jfac_nfw.txt','w')
        for i in range(len(Jstore)):
            f.write('%f\n' % Jstore[i])
        f.close()
    if (calc_Dfac == 'yes'):
        f = open(outdir+'output_Dfac_nfw.txt','w')
        for i in range(len(Dstore)):
            f.write('%f\n' % Dstore[i])
        f.close()                                   

    #And write the distance data:
    f = open(outdir+'output_dstore_nfw.txt','w')
    for i in range(len(dstore)):
        f.write('%f\n' % dstore[i])
    f.close()
    ...
    plt.ylim([0,np.max(n)])
    plt.savefig(outdir+'output_d_nfw.pdf',bbox_inches='tight')
    ...
    if (calc_Jfac == 'yes'):
        ...
        plt.ylabel(r'$N$',fontsize=myfontsize)
        plt.ylim([0,np.max(n)])
        plt.savefig(outdir+'output_Jfac_nfw.pdf',bbox_inches='tight')
    ...
    if (calc_Dfac == 'yes'):
        ...
        plt.ylabel(r'$N$',fontsize=myfontsize)
        plt.ylim([0,np.max(n)])
        plt.savefig(outdir+'output_Dfac_nfw.pdf',bbox_inches='tight')
    ...
    if (virialshape == 'yes'):
        ...
        plt.ylabel(r'$N$',fontsize=myfontsize)
        plt.ylim([0,np.max(n)])
        plt.savefig(outdir+'output_vs1_nfw.pdf',bbox_inches='tight')
        ...
        plt.ylabel(r'$N$',fontsize=myfontsize)
        plt.ylim([0,np.max(n)])
        plt.savefig(outdir+'output_vs2_nfw.pdf',bbox_inches='tight')
    ...
    plt.savefig(outdir+'output_M200_nfw.pdf',bbox_inches='tight')
    ...
    plt.savefig(outdir+'output_vmax_nfw.pdf',bbox_inches='tight')
    ...
    plt.savefig(outdir+'output_c200_nfw.pdf',bbox_inches='tight')
   ...
   #Calculate M200 +/- 68%:
    ...
    f = open(outdir+'output_M200vals_nfw.txt','w')
    ...
    f = open(outdir+'output_vmaxvals_nfw.txt','w')
    f.write('%f %f %f %f %f %f %f\n' % \
            (vmaxmed, vmaxsixlow, vmaxsixhi,\
             vmaxninelow, vmaxninehi, \
