...
#a "Scalar Field Dark Matter" spherical dark matter halo and some 
...
def lnprior_set_single(theta,n_betpars,bet0min,bet0max,\
                       ...
                       clow,chigh,logrTFlow,logrTFhigh,\
    ...
    minarr[n_betpars+nu_components*2+2] = logrTFlow
    maxarr[n_betpars+nu_components*2+2] = logrTFhigh
    minarr[n_betpars+nu_components*2+3] = logMcenlow
    maxarr[n_betpars+nu_components*2+3] = logMcenhigh
    minarr[n_betpars+nu_components*2+4] = acenlow
    maxarr[n_betpars+nu_components*2+4] = acenhigh
    minarr[n_betpars+nu_components*2+5] = Arotlow
    maxarr[n_betpars+nu_components*2+5] = Arothigh
    minarr[n_betpars+nu_components*2+6] = drangelow
    maxarr[n_betpars+nu_components*2+6] = drangehigh
...
#MUSE
from gravsphere_initialise_AntliaB import *
...
print('rTFlow, rTFhigh:', 10.0**logrTFlow, 10.0**logrTFhigh)
if (logMcenlow > 0.0):
...
M = lambda r, Mpars: \
    sfdm_mass(r,Mpars[0],Mpars[1],Mpars[2])
    
rho = lambda r, Mpars: \
    sfdm(r,Mpars[0],Mpars[1],Mpars[2])
    
dlnrhodlnr = lambda r, Mpars: \
    sfdm_dlnrhodlnr(r,Mpars[0],Mpars[1],Mpars[2])
    
#Central dark mass:
Mcentral = lambda r, Mpars: \
           plummass(r,Mpars[3:5])
...
if (codemode == 'run'):
    ...
    pos[:,n_betpars+nu_components*2+2] = \
        np.random.uniform(logrTFlow,logrTFhigh,nwalkers)
    ...
    f = open(outdir+'Boutput_chain_sfdm.txt','w')
...
#Plotting code:
    ...
    #Read in the data:
    data_in = \
        np.genfromtxt(outdir+'output_surfden_sfdm.txt',dtype='f8')
    rbin_phot = data_in[:,0]
    surfden = data_in[:,1]
    surfdenerr = data_in[:,2]
    data_in = \
        np.genfromtxt(outdir+'output_sigp_sfdm.txt',dtype='f8')
    rbin_kin = data_in[:,0]
    sigpmean = data_in[:,1]
    sigperr = data_in[:,2]
    ...
    if (propermotion == 'yes'):
        data_in = \
            np.genfromtxt(outdir+'output_prop_sfdm.txt',dtype='f8')
        rbin_kinp = data_in[:,0]
        sigpmr = data_in[:,1]
        sigpmrerr = data_in[:,2]
        sigpmt = data_in[:,3]
    ...
    data_in = \
        np.genfromtxt(outdir+'Boutput_chain_sfdm.txt',dtype='f8')
    chisq = data_in[:,0]
    ...
    for i in range(nsamples):
        ...
        McenMstore[i] = Mparsu[3]
        Mcenastore[i] = Mparsu[4]
...
#And now make the plots:
    ...
    plt.savefig(outdir+'output_Sigstar_sfdm.pdf',bbox_inches='tight')
    ...
    plt.savefig(outdir+'output_sigLOS_sfdm.pdf',bbox_inches='tight')
    ...
    if (propermotion == 'yes'):
        ...
        plt.savefig(outdir+'output_sigpmr_sfdm.pdf',bbox_inches='tight')
        ...
        plt.savefig(outdir+'output_sigpmt_sfdm.pdf',bbox_inches='tight')
    ...
    plt.savefig(outdir+'output_beta_sfdm.pdf',bbox_inches='tight')

    #Write the above data to files for comparitive plotting later:
    f = open(outdir+'output_bet_sfdm.txt','w')
    ...
    plt.savefig(outdir+'output_betastar_sfdm.pdf',bbox_inches='tight')

    #Write the above data to files for comparitive plotting later:
    f = open(outdir+'output_betstar_sfdm.txt','w')
    for i in range(len(rbin)):
        f.write('%f %f %f %f %f %f %f %f\n' % \
    ...
    plt.savefig(outdir+'output_Mass_sfdm.pdf',bbox_inches='tight')

    #Write the above data to files for comparitive plotting later:
    f = open(outdir+'output_M_sfdm.txt','w')
    for i in range(len(rbin)):
        f.write('%f %f %f %f %f %f %f %f\n' % \
    ...
    f = open(outdir+'output_MdynMstar_sfdm.txt','w')
    ...
    #And nu_mass_r:
    f = open(outdir+'output_nu_mass_r_sfdm.txt','w')
    for i in range(len(Mstar_rad)):
        f.write('%f %f %f %f %f %f %f %f\n' % \
    ...
    #And Mstar:
    f = open(outdir+'output_Mass_Mstar_sfdm.txt','w')
    for i in range(len(Mstar_rad)):
        f.write('%f %f %f %f %f %f %f %f\n' % \
    ...
    f = open(outdir+'output_Mass_Mcen_sfdm.txt','w')
    for i in range(len(rbin)):
        f.write('%f %f %f %f %f %f %f %f\n' % \
    ...
    plt.savefig(outdir+'output_rho_sfdm.pdf',bbox_inches='tight')

    #Write the above data to files for comparitive plotting later:
    f = open(outdir+'output_rho_sfdm.txt','w')
    for i in range(len(rbin)):
        f.write('%f %f %f %f %f %f %f %f\n' % \
    ...
    #And the SFDM parameters:
    f = open(outdir+'output_M200c200_chain_sfdm.txt','w')
    for i in range(len(M200store)):
        f.write('%f %f %f %f\n' % \
                (M200store[i],cstore[i],vmaxstore[i],rTFstore[i]))
    f.close()
    ...
    plt.savefig(outdir+'output_dlnrhodlnr_sfdm.pdf',bbox_inches='tight')

    #Write the above data to files for comparitive plotting later:
    f = open(outdir+'output_dlnrhodlnr_sfdm.txt','w')
    for i in range(len(rbin)):
        f.write('%f %f %f %f %f %f %f %f\n' % \
    ...
    plt.savefig(outdir+'output_vphirot_sfdm.pdf',bbox_inches='tight')
    
    #######################################################
    #Additional write output
    #And write the D+J-factor data:
    if (calc_Jfac == 'yes'):
        f = open(outdir+'output_Jfac_sfdm.txt','w')
        for i in range(len(Jstore)):
            f.write('%f\n' % Jstore[i])
        f.close()
    if (calc_Dfac == 'yes'):
        f = open(outdir+'output_Dfac_sfdm.txt','w')
        for i in range(len(Dstore)):
            f.write('%f\n' % Dstore[i])
        f.close()                                   

    #And write the distance data:
    f = open(outdir+'output_dstore_sfdm.txt','w')
    for i in range(len(dstore)):
        f.write('%f\n' % dstore[i])
    f.close()
    ...
    plt.ylim([0,np.max(n)])
    plt.savefig(outdir+'output_d_sfdm.pdf',bbox_inches='tight')
    ...
    if (calc_Jfac == 'yes'):
        ...
        plt.ylabel(r'$N$',fontsize=myfontsize)
        plt.ylim([0,np.max(n)])
        plt.savefig(outdir+'output_Jfac_sfdm.pdf',bbox_inches='tight')
    ...
    if (calc_Dfac == 'yes'):
        ...
        plt.ylabel(r'$N$',fontsize=myfontsize)
        plt.ylim([0,np.max(n)])
        plt.savefig(outdir+'output_Dfac_sfdm.pdf',bbox_inches='tight')
    ...
    if (virialshape == 'yes'):
        ...
        plt.ylabel(r'$N$',fontsize=myfontsize)
        plt.ylim([0,np.max(n)])
        plt.savefig(outdir+'output_vs1_sfdm.pdf',bbox_inches='tight')
        ...
        plt.ylabel(r'$N$',fontsize=myfontsize)
        plt.ylim([0,np.max(n)])
        plt.savefig(outdir+'output_vs2_sfdm.pdf',bbox_inches='tight')

    ##### SFDM model parameters #####
    nbin = 15
    fig = plt.figure(figsize=(figx,figy))
    ax = fig.add_subplot(111)
    for axis in ['top','bottom','left','right']:
    ...
    plt.savefig(outdir+'output_M200_sfdm.pdf',bbox_inches='tight')
    ...
    plt.savefig(outdir+'output_vmax_sfdm.pdf',bbox_inches='tight')
    ...
    plt.savefig(outdir+'output_c200_sfdm.pdf',bbox_inches='tight')
    ...
    plt.xticks(fontsize=myfontsize)
    plt.yticks(fontsize=myfontsize)
    ax.xaxis.set_ticks(np.arange(logrTFlow,logrTFhigh+1.0,1.0))
    
    n, bins, patches = plt.hist(np.log10(rTFstore),bins=nbin,\
                                range=(logrTFlow,logrTFhigh),\
                                facecolor='k', \
                                histtype='bar',alpha=0.5)
        
    plt.xlabel(r'${\rm Log}_{10}[R_{TF}/{\rm kpc}]$',fontsize=myfontsize)
    plt.xlim([logrTFlow,logrTFhigh])

    plt.ylabel(r'$N$',fontsize=myfontsize) 
    plt.ylim([0,np.max(n)])
    plt.savefig(outdir+'output_rTF.pdf',bbox_inches='tight')
   ...
   #Calculate M200 +/- 68%:
    ...
    f = open(outdir+'output_M200vals_sfdm.txt','w')
    ...
    f = open(outdir+'output_vmaxvals_sfdm.txt','w')
    f.write('%f %f %f %f %f %f %f\n' % \
            (vmaxmed, vmaxsixlow, vmaxsixhi,\
             vmaxninelow, vmaxninehi, \
